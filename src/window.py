...
class Window(Adw.ApplicationWindow):
    ...
    def __init__(self, app, launch_uri):
        ...
        langlinks_stack_page = self.panel_stack.add_named(
            self.langlinks_panel, "langlinks"
        )

        self.bookmarks_panel = BookmarksPanel(self)
        bookmarks_stack_page = self.panel_stack.add_named(
            self.bookmarks_panel, "bookmarks"
        )
        ...
        settings.bind(
            "panel-pinned", self.panel_button, "active", Gio.SettingsBindFlags.DEFAULT
        )
        settings.bind(
            "panel-page",
            self.panel_stack,
        ...
        self.breakpoint_window_medium.connect(
            "unapply", self._breakpoint_medium_unapply_cb
        )
        self.breakpoint_window_small.connect("apply", self._breakpoint_small_apply_cb)
        self.breakpoint_window_small.connect(
            "unapply", self._breakpoint_small_unapply_cb
        )
        ...
        self.search_button.connect(
            "toggled", self._bar_selector_button_toggled_cb, "search"
        )
        self.langlinks_button.connect(
            "toggled", self._bar_selector_button_toggled_cb, "langlinks"
        )
        self.bookmarks_button.connect(
            "toggled", self._bar_selector_button_toggled_cb, "bookmarks"
        )
        self.history_button.connect(
            "toggled", self._bar_selector_button_toggled_cb, "history"
        )
        ...
        gesture = Gtk.GestureClick(
            button=0, propagation_phase=Gtk.PropagationPhase.CAPTURE
        )
    ...
    def _set_actions(self, app):
        actions = [
            ("prev-page", self._prev_page_cb, ("<Alt>Left",)),
        ...
        pin_panel_action = Gio.SimpleAction.new_stateful(
            "pin-panel", None, GLib.Variant.new_boolean(False)
        )
    ...
    def _tabview_selected_page_cb(self, tabview, value):
        ...
        if self.page.lazy_load:
            ...
            self.toc_panel.populate(
                self.page.wikiview.title, self.page.wikiview.sections
            )
    ...
    def _print_page_cb(self, action, parameter):
        ...
        # Force light theme to avoid blank page
    ...
    def _show_print_dialog(self):
        ...
        if result == WebKit.PrintOperationResponse.PRINT:
            handler_finished = print_operation.connect(
                "finished", self._print_operation_finished
            )
            print_operation.connect(
                "failed", self._print_operation_failed, handler_finished
            )
        else:
